...
const carouselThumbnail = document.getElementById('main-thumbnail-carousel');
let galleryList = '<i class="fa-solid fa-circle-chevron-left" id="prev-arrow"></i><ul>';
let thumbnailList = '<ul>';
...
for (let i = 0; i < images.length; i++){
    ...
    thumbnailList += `<li><img src="${images[i]}" alt="Gallery Image ${i + 1}"></li>`;
}
...
carouselThumbnail.innerHTML = thumbnailList;
...
const thumbnailListImages = document.querySelectorAll('#main-thumbnail-carousel img');
let activeClassIndex = 0;

// 2. Rendo visibile la prima immagine dell'array inserendo la classe CSS "active"
galleryListImages[activeClassIndex].classList.add('active');
thumbnailListImages[activeClassIndex].classList.add('active');
...
nextArrow.addEventListener('click', function(){
    ...
    thumbnailListImages[activeClassIndex].classList.remove('active');
...
// ! BONUS 2: Animare il carosello delle thumbnail, connettendolo allo stesso array della main-gallery